    ...
    return (
        <div className="bg-[#eff6ff] p-3 px-5 border-b border-blue-100 flex justify-between items-center z-10 w-full animate-in fade-in slide-in-from-top-2 relative shadow-inner">
            <div className="flex items-center text-sm font-bold text-blue-800 tracking-tight">
               <CheckSquare className="w-4 h-4 mr-2 text-primary-accent" />
               {selectedCount} Matrix Node{selectedCount !== 1 ? 's' : ''} Selected
            </div>
            <div className="flex items-center gap-3">
               <button 
                  onClick={() => onActionTrigger("Reviewed")}
                  className="text-xs font-bold tracking-tight px-3 py-1.5 bg-surface border border-primary-accent-light text-primary-accent-dark rounded-lg shadow-sm hover:bg-blue-50 transition-colors flex items-center h-8"
               >
                  <Settings className="w-3.5 h-3.5 mr-1.5 opacity-70" /> Flag Reviewed Array
               </button>
               <button 
                  onClick={() => onActionTrigger("Resolved")}
                  className="text-xs font-bold tracking-tight px-3 py-1.5 bg-primary-accent border border-emerald-700/50 text-white rounded-lg shadow-sm hover:bg-emerald-700 hover:shadow transition-all flex items-center h-8"
               >
