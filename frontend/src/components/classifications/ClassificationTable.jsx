        ...
        <table className="w-full text-left border-collapse text-sm">
          ...
          <tbody className="divide-y divide-gray-50 bg-white group-trigger">
               {data.map((row) => (
                 ...
                 >
                   <td className="px-5 py-3.5 text-xs font-mono text-gray-400 tracking-wider font-semibold">
                       #{row._id.slice(-6).toUpperCase()}
                   </td>
                   <td className="px-5 py-3.5">
                       <div className="font-bold text-gray-900 truncate max-w-[200px]">{row.leakage_type}</div>
                   </td>
                   <td className="px-5 py-3.5">
                       <div className="flex items-center justify-center space-x-2">
                           <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                                <div className="h-full bg-indigo-500 rounded-full" style={{width: `${row.confidence_score * 100}%`}}></div>
                           </div>
                           <span className="text-[11px] font-bold text-indigo-500 font-mono tracking-tighter">{(row.confidence_score * 100).toFixed(0)}%</span>
                       </div>
                   </td>
                   <td className="px-5 py-3.5 text-slate-500 text-[11px] font-medium tracking-wide hidden md:table-cell truncate max-w-[250px]">
                       {row.root_cause}
                   </td>
                   <td className="px-5 py-3.5 text-right font-bold text-emerald-600 whitespace-nowrap tracking-tight">
                   ...
                   <td className="px-5 py-3.5 text-right">
                       <button className="p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-white rounded-lg transition-all border border-transparent hover:border-indigo-200 hover:shadow-sm">
