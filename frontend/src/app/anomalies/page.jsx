...
export default function AnomaliesListPage() {
  ...
  return (
      <div className="min-h-[calc(100vh-64px)] bg-[#F8FAFC] pb-12 selection:bg-blue-100 font-sans">
         <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 pt-8">
            <div className="flex justify-between items-center mb-8">
                <div>
                   ...
                   <p className="text-sm text-gray-500 mt-1">Review flagged algorithmic multi-layer transactions instantly.</p>
                </div>
                <button 
                  onClick={() => queryClient.invalidateQueries(['anomalies'])}
                  className="flex items-center text-sm font-medium text-gray-600 bg-white border border-gray-200 px-4 py-2 rounded-lg shadow-sm hover:bg-gray-50 transition"
                  disabled={isFetching}
                >
                    <RefreshCw className={`w-4 h-4 mr-2 ${isFetching ? 'animate-spin text-blue-500' : ''}`} /> Sync Database Arrays
                </button>
            </div>
                ...
                <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm bg-gray-50/50 mt-auto">
                    ...
                    <div className="flex space-x-2">
                        <button 
                           ...
                           className="px-4 py-1.5 border border-gray-200 bg-white hover:bg-gray-50 rounded-lg text-gray-700 disabled:opacity-50 transition-colors shadow-sm font-semibold"
                        >
                           Previous Segment
                        </button>
                        <button 
